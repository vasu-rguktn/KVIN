...
def build_dataset(df, part_name, training=True):
    # df["image_path"].astype(str).values
    ds = tf.data.Dataset.from_tensor_slices(
        (df["image_path"].values, df["label"].values)
    )
    ds = ds.map(
        lambda x, y: parse_image(x, y, training=training), num_parallel_calls=tf.data.AUTOTUNE
    )
    # filename=f"cache_{part_name}_{split}.tf"
    ds = ds.cache(f"cache_{part_name}_freeze_no_aug.tf")
...
def train_part(part_name):
    ...
    # Evaluate
    # -----------------------------
    model.save(os.path.join(THIS_FOLDER, "weights", f"ResNet50_{part_name}_frac_freeze_no_aug.keras"))
