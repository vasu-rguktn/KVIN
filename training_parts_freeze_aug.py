...
image_dir = os.path.join(THIS_FOLDER, "Dataset-Train")
...
def build_dataset(df, training=True):
    ...
    ds = ds.map(lambda x, y: parse_image(x, y, training=training), tf.data.AUTOTUNE)

    ds = ds.cache(filename="cache_freeze_aug.tf")
...
model.save("weights/ResNet50_BodyParts_freeze_aug.keras")
...
plt.savefig(os.path.join(plot_dir, "confusion_matrix_freeze_aug.jpeg"))
plt.close()

cm_df = pd.DataFrame(cm, index=le.classes_, columns=le.classes_)
cm_df.to_csv(os.path.join(plot_dir, "confusion_matrix_freeze_aug.csv"))
...
plt.legend()
plt.grid(True)
plt.savefig(os.path.join(plot_dir, "_Metrics_freeze_aug.jpeg"))
plt.close()
...
plt.grid(True)
plt.savefig(os.path.join(plot_dir, "_Loss_freeze_aug.jpeg"))
plt.close()
