...
def build_dataset(df, training=True):
    ...
    ds = ds.cache(filename="cache_freeze_no_aug.tf")
...
model.save("weights/ResNet50_BodyParts_freeze_no_aug.keras")
...
plt.savefig(os.path.join(plot_dir, "confusion_matrix_freeze_no_aug.jpeg"))
plt.close()

cm_df = pd.DataFrame(cm, index=le.classes_, columns=le.classes_)
cm_df.to_csv(os.path.join(plot_dir, "confusion_matrix_freeze_no_aug.csv"))
...
plt.legend()
plt.grid(True)
plt.savefig(os.path.join(plot_dir, "_Metrics_freeze_no_aug.jpeg"))
plt.close()
...
plt.grid(True)
plt.savefig(os.path.join(plot_dir, "_Loss_freeze_no_aug.jpeg"))
plt.close()
